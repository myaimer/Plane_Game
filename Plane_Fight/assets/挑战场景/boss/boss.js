...
cc.Class({
    ...
    boom(){
        ...
        for(let i = 0; i < leafNode.length; i++){
            // 移除包围盒
            leafNode[i].removeComponent(cc.PolygonCollider);
            // 添加碰撞组件
            leafNode[i].addComponent(cc.PhysicsBoxCollider);
            // 把碰撞组件的类型改为刚体的静态类型
            leafNode[i].getComponent(cc.RigidBody).type = cc.RigidBodyType.Static;
            let flyDis = cc.v2(20,0);
            let r = Math.PI * 2 / leafNode.length;
            let v = flyDis.rotate(r * i);
            // 给碰撞组件一个初始的向量力，这样的话在类型改为自由的时候就会爆炸飞出去
            leafNode[i].getComponent(cc.RigidBody)._b2Body.m_linearVelocity.x = v.x;
            leafNode[i].getComponent(cc.RigidBody)._b2Body.m_linearVelocity.y = v.y;
